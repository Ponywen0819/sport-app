...
import type { BodyIndexSchemaCheckResponse } from "@/app/api/notion/body-index/schema/route";

async function checkSchema(): Promise<BodyIndexSchemaCheckResponse> {
  const res = await fetch("/api/notion/body-index/schema");
  if (!res.ok) throw new Error(`${res.status}`);
  return res.json();
}

async function migrateSchema(): Promise<void> {
  const res = await fetch("/api/notion/body-index/schema", { method: "POST" });
  if (!res.ok) throw new Error(`${res.status}`);
}
 ...
 * - Schema 正常 / 修復後 → 渲染子元件
 */
export const BodyIndexSchemaGate = ({ children }: Props) => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["body-index-schema-check"],
    queryFn: checkSchema,
    staleTime: Infinity,
    retry: 1,
  });
  ...
  const migrateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["body-index-schema-check"] });
      setMigrated(true);
    },
  });
    ...
    return (
      <div className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[0, 1, 2, 3].map((i) => (
            <div key={i} className="h-24 bg-stone-800 rounded-2xl animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  if (!data || data.allValid || migrated) {
    return (
      <>
        {migrated && (
          <div className="mb-3 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl px-4 py-3 flex items-center gap-3">
  ...
  const missing = data.bodyIndexes.missingProperties;

  return (
    <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-2xl px-4 py-3 flex flex-col gap-3">
      <div className="flex items-start gap-3">
        ...
        <div className="flex flex-col gap-1 min-w-0">
          ...
          <div className="mt-1">
            <p className="text-stone-400 text-xs font-medium">{data.bodyIndexes.dbName}</p>
